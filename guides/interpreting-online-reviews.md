---
layout: guide
title: "How to Interpret Online Reviews of Senior Care Facilities"
description: "Where senior living star ratings actually come from, why review sites disagree, and how to read online reviews without being misled either direction."
permalink: /guides/interpreting-online-reviews/
topic_group: quality
updated: 2026-07-12
related:
  - /guides/reading-inspection-reports-and-ratings/
  - /guides/checking-complaints-and-state-records/
  - /guides/warning-signs-when-choosing-a-facility/
  - /guides/touring-checklist/
faqs: [{"q": "A facility asked my relative to leave a review. Is that shady?", "a": "No — ethically asking satisfied families for reviews is standard and fair, since angry people review unprompted. What's shady is gating (only steering happy families to public sites), paying for reviews, or pressuring residents."}, {"q": "Two sites show very different ratings for the same community. Which is right?", "a": "Neither is \"right\" — they're different populations of reviewers on platforms with different incentives. Note both, read the text on each, and let the official inspection record be the tiebreaker."}, {"q": "Should I trust a community with no reviews at all?", "a": "Small homes and new communities often have few or none — that's not a red flag by itself, but it shifts all the weight onto the state record, references, and your own visits."}, {"q": "Why do you show review scores on your listings at all?", "a": "Because families use them anyway — so we'd rather show the number with its source, count, and date, alongside the official records, than pretend the stars don't exist. Every community in our directory cleared a review-evidence or CMS-rating standard, and the evidence is printed on the page."}]
---

Type any senior community's name into a search engine and you'll get a wall of stars — a 4.6
here, a 3.9 there, sometimes for the same building. Before you let a number make this decision
for you, it's worth knowing where each kind of rating comes from, who wrote the reviews, and
what they can and can't tell you. On our own listings we show review evidence with its source
and date for exactly this reason — so you know what you're looking at.

## Know your sources — they are not the same thing

**Google reviews** are written by anyone with a Google account — family members, employees,
former employees, vendors, sometimes people who visited once. Nobody verifies that the
reviewer ever set foot in the building. Strengths: large volume, hard for a facility to
curate, recency is visible. Weaknesses: no verification, and both extremes are overrepresented.

**Referral-platform reviews** (A Place for Mom, Caring.com, SeniorAdvisor) sit on sites that
are paid by facilities for placements. The reviews themselves are often collected from
families the platform referred, which skews toward people who just moved in — the honeymoon
period — and the platform has a business relationship with the community being reviewed. Use
the text; hold the stars loosely. A 2024 Washington Post analysis found top-recommended
facilities on the largest platform coexisting with substandard-care citations.

**Facility-website testimonials** are marketing. Real quotes, hand-picked. Fine to read,
meaningless as evidence.

**CMS star ratings** (nursing homes only) are the odd one out: they aren't reviews at all.
They're built from government inspections, payroll-based staffing data, and clinical quality
measures. That's why we treat them as the primary quality signal for skilled nursing — and
why we explain [what the stars mean and where they fall short](/guides/reading-inspection-reports-and-ratings/)
separately from consumer reviews.

## Why good facilities can carry mediocre scores

Keep three distortions in mind before judging any number:

- **Who writes reviews:** people who are delighted or furious. The hundreds of quietly
  satisfied families in the middle mostly don't. A community with 4.2 across 80 reviews may
  be functionally identical to one with 4.6 across 15.
- **The rehab effect:** [skilled nursing facilities](/guides/skilled-nursing/) score
  structurally lower everywhere, because many reviewers are families going through a
  frightening post-hospital stay. A 4.0 nursing home may be outperforming a 4.5 assisted
  living community, relative to the job it's doing.
- **Small-sample noise:** a 5.0 built on three reviews is a coin flip, not a track record.
  That's why our own listings require multiple reviews before we treat a rating as evidence
  — one glowing review tells you someone's cousin was pleased.

## How to actually read reviews

1. **Read the recent ones first.** A community under new management is not the community
   from a 2019 review — in either direction.
2. **Look for repeated specifics, not star math.** Three separate reviews mentioning slow
   call-light response is signal. One furious essay about a billing dispute is a data point.
3. **Read the management responses.** Specific, non-defensive replies to criticism are one
   of the best cheap indicators of a well-run building. Copy-paste apologies — or silence —
   tell you something too.
4. **Discount the extremes, mine the middle.** Three-star reviews are usually written by the
   most honest reviewers; they contain the trade-offs you'll actually live with.
5. **Then verify against the official record.** Reviews are impressions;
   [state inspections and complaint records](/guides/checking-complaints-and-state-records/)
   are investigated facts. When reviews and records disagree, weight the records — and ask
   the administrator about both on your [tour](/guides/touring-checklist/).

> **Ask this:** "Your online reviews mention [specific recurring issue] — what's the story,
> and what changed?" You'll learn more from how they answer than from the stars themselves.

## Common questions

**A facility asked my relative to leave a review. Is that shady?**
No — ethically asking satisfied families for reviews is standard and fair, since angry people
review unprompted. What's shady is gating (only steering happy families to public sites),
paying for reviews, or pressuring residents.

**Two sites show very different ratings for the same community. Which is right?**
Neither is "right" — they're different populations of reviewers on platforms with different
incentives. Note both, read the text on each, and let the official inspection record be the
tiebreaker.

**Should I trust a community with no reviews at all?**
Small homes and new communities often have few or none — that's not a red flag by itself,
but it shifts all the weight onto the state record, references, and your own visits.

**Why do you show review scores on your listings at all?**
Because families use them anyway — so we'd rather show the number with its source, count, and
date, alongside the official records, than pretend the stars don't exist. Every community in
our directory cleared a review-evidence or CMS-rating standard, and the evidence is printed
on the page.
